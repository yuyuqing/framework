...
#include "dpdk_device_bkq.h"
#include "base_log.h"


DEFINE_DEVICE(CBKQDevice);


SWORD32 CBKQDevice::LsiEventCallBack(WORD16          wPortID,
...
CBKQDevice::CBKQDevice (const T_DeviceParam &rtParam)
    : CBaseDevice(E_BKQ_DEVICE, rtParam)
{
...
CBKQDevice::~CBKQDevice()
{
}


/* 调用派生类设置RxConf属性 */
WORD32 CBKQDevice::SetRxConf()
{
    return SUCCESS;
}
...
/* 调用派生类设置TxConf属性 */
WORD32 CBKQDevice::SetTxConf()
{
    return SUCCESS;
}


WORD32 CBKQDevice::Initialize()
{
    TRACE_STACK("CBKQDevice::Initialize()");
    ...
    WORD32 dwResult = CBaseDevice::Initialize(CBKQDevice::LsiEventCallBack);
    if (SUCCESS != dwResult)
    {
        return FAIL;
    }
